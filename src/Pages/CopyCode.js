    ...
    const fetchConfigId = async () => {
      try {
        ...
        if (response.data.success && response.data.config_id) {
          ...
          const script = `<script src="https://cdn.jsdelivr.net/gh/boyhaimai/model_admin_just_chat_v17@main/dist/model_admin_just_chat.js" data-server-url="http://localhost:5000" data-id-config="${configId}" defer></script>`;
